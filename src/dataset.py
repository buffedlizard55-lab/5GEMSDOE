...
def _maybe_add_aux_features(X, meta, aux_paths=None):
    """Append grid-aligned auxiliary feature rasters (session 5GEMSDOE-1, 2026-09-25).

    ``aux_paths`` is a list of GeoTIFFs that already sit on the EXACT competition grid
    (same shape, CRS and transform as training_features.tif) - e.g. the 100 m topographic
    scarp channel written by ``scripts/build_topo_features.py`` from the seamless 10 m 3DEP
    DEM, or the GeoDAWN radiometric grids resampled by the same script.  The check is strict on
    purpose: a raster that is merely "close" would be a silent train/inference channel drift.
    Band tags are read from each file so ``band_names`` can name them.
    """
    if not aux_paths:
        return X, []
    import rasterio
    ...
    for ap in aux_paths:
        ap = Path(ap)
        if not ap.is_file():
            raise FileNotFoundError(f"data.aux_feature_paths entry not found: {ap} "
                                    f"(build it with scripts/build_topo_features.py)")
        with rasterio.open(ap) as ds:
            if (ds.height, ds.width) != (H, W):
                raise ValueError(f"{ap}: grid {ds.height}x{ds.width} != feature grid {H}x{W}")
            if meta.get("transform") is not None and tuple(ds.transform)[:6] != tuple(meta["transform"])[:6]:
                raise ValueError(f"{ap}: transform {tuple(ds.transform)[:6]} != feature transform "
                                 f"{tuple(meta['transform'])[:6]}")
            if meta.get("crs") is not None and ds.crs is not None and ds.crs != meta["crs"]:
                raise ValueError(f"{ap}: crs {ds.crs} != feature crs {meta['crs']}")
            arr = ds.read().astype(np.float32)          # (C,H,W)
            if ds.nodata is not None and np.isfinite(ds.nodata):
                arr[arr == ds.nodata] = np.nan
            for i in range(ds.count):
                t = dict(ds.tags(i + 1))
                t.setdefault("description", ds.descriptions[i] or f"{ap.stem}_b{i + 1}")
                t.setdefault("band_name", ds.descriptions[i] or f"{ap.stem}_b{i + 1}")
                tags.append(t)
            extra.append(np.moveaxis(arr, 0, -1))
    return np.concatenate([X.astype(np.float32, copy=False)] + extra, axis=-1), tags
...
def load_features_and_labels(feature_path=None, label_path=None, require_labels=True,
                             ...
                             external_dem_path=None, aux_feature_paths=None):
    ...
    if len(tags) < X.shape[-1]:
        tags = list(tags) + [{"description": n} for n in
                             ("dem_slope", "dem_curvature", "dem_tpi", "dem_tri", "dem_detrended")]
    X, aux_tags = _maybe_add_aux_features(X, fmeta, aux_feature_paths)
    tags = list(tags) + aux_tags
    if lp is None:
