...
### Next steps, in order (for session 2)
1. **Read the `build-topo-features` run that this branch's first push fired** (Actions → build-topo-features). If it
   landed: `data/evidence/topo/topo_features_100m.report.json` is committed and `docs/strategy.html` §6 shows coverage.
   Download the `topo_features_100m` artifact into `data/external/`. If it failed: the log says which step (S3 probe,
   build, stack-verify) — fix and re-fire by touching `.github/triggers/build-topo-features`.
2. **Train the paired comparison** `configs/config_topo.yaml` vs `configs/config.yaml` on the same folds (the
   `train-ensemble.yml` runner path GEMSDOE used: ≈300 min/fold CPU; a GPU box is far better). The only difference between
   the two configs is `data.aux_feature_paths`. Compare held-out DTI per fold; decide H1 on the paired difference.
3. **Upload the density probe once** (`scripts/density_probe.py`; `outputs/density_probe/gems-density-probe-*.tif` — Note is
   printed by the script). Paste the returned score into `scripts/density_probe.py --invert SCORE` and record |G| in
   `data/evidence/leaderboard_anchor/` (add a `probe` entry); re-run `leaderboard_anchor.py` so every T_i(G) line becomes a point.
4. **H2 radiometrics**: a runner step that downloads `22103_area2_tiffs.zip` / `22103_area1_tiffs.zip` from ScienceBase
   (E43), reprojects K/eTh/eU/TC to the competition grid with the same aux hook, and appends to `aux_feature_paths`.
5. **Budget by the rule (H4)**: implement a re-budgeting script (not yet written) — given a calibrated probability field and |G|, emit
   in decreasing expected-hit-rate order until the marginal rate crosses `0.2·DTI/(1−0.2·DTI)`; replaces floor sweeps.
6. Confirm the 0.1560 file identity from the smashi34 submissions page and edit `SCORED` in `scripts/leaderboard_anchor.py`.
7. Owner decision (flagged, not resolvable here): multiple accounts from one project family in one competition vs the
   official rules PDF.

### What was executed this session
1. **Full copy of GEMSDOE** (all code, docs, evidence, the sha256-pinned data bridge) into this repository; every
   `buffedlizard55-lab/GEMSDOE` / `github.io/GEMSDOE/` reference re-pointed to `5GEMSDOE` except (a) the anchor script's
   provenance links to the sibling sites and (b) two rows of the verified-link CSV whose HTTP-200 record belongs to the
   original repo. Push-fired triggers for fetch/place/train/ensemble/pseudo-label/verify were removed (a fresh repo would
   have fired all of them on its first push); every workflow remains dispatchable. `make-submission` keeps its trigger
   (a test requires it, and it is the fast validating route).
2. **Official rasters placed and verified** from the bridge (`assemble_data_bridge.py` OK, `prepare_data.py` OK:
   3292×3730, 19 bands, EPSG:32611, 100 m).
3. **Five scored files retrieved and measured** (`scripts/leaderboard_anchor.py --check` → PASS): sha256 of every file
   equals the name it was published under; all seven candidate files are template-conformant (0 NaN inside, 0 finite
   outside). Known-fault pixels 60,988 = 1.18 % of 5,167,373 valid. The 0.1563 file is 78 % >3 px off-catalogue;
   the GEMSDOE2 union is a strict superset (+10,668 px, +9,430 unmasked) that scored −0.0003; the Pindrop fields are
   nearly disjoint from the CNN (Jaccard 0.02–0.07). Deductions D1–D7 are in the JSON and on `docs/strategy.html`.
4. **Metric algebra derived and checked against the board**: DTI = T/(0.2(T+F)+0.8G); binary optimal; marginal rule
   ΔT > 0.2·DTI·ΔF/(1−0.2·DTI); the union pair's implied marginal hit-rate (0.028–0.030) equals the break-even at
   0.156 (0.032) — the rule reproduces a real leaderboard delta.
5. **Density probe** built and validated (`gems-density-probe-…-9cdae9b4.tif`, 527,755 B; `validate_submission.py` PASSED;
   `submission_io validate-conformant` conformant; inversion table written).
6. **Topographic scarp channel** implemented (`scripts/build_topo_features.py`, 9 bands, 10 m → 100 m statistics; 5 unit
   tests incl. an end-to-end CLI run on a synthetic scarp on a template grid — a 4 m step and a 2.3° plane of equal relief
   separate 4.4× in slope_max, 8× in hillshade contrast); generic `data.aux_feature_paths` hook added to
   `src/dataset.py` and wired into both `src/train.py` and `src/inference.py` (strict grid check, tags carried);
   `configs/config_topo.yaml` (identical to `config.yaml` except the aux channel); `.github/workflows/build-topo-features.yml`
   (S3 probe → build → stack-verify → artifact → report commit).
7. **Site**: new `docs/strategy.html` (rendered from the anchor JSON), a "Site 5" banner ahead of the builder on the landing
   and executive pages, nav entry "★ Strategy 5". `scripts/audit_docs.py` PASS after adding catalog rows E40–E43, C14–C17.
8. **Tests**: 447 passed / 2 skipped locally (torch-dependent modules run in CI's Tests workflow), 5 new tests.

### Verified sources used this session (all fetched 2026-09-25)
problem page 967 (§competition structure, §metric, §submission format); forum 11516 (masking), 11536 (new-fault
definition), 11527/7 (no disclosure); leaderboard; ScienceBase 657e1d85d34e23d3533209f7 (GeoDAWN files table);
ScienceBase 4f70aa9fe4b058caae3f8de5 + data.usgs.gov 3a81321b… (3DEP 1/3″); Hermant et al. SGW 2025 (PDF).

### Irregularities flagged
- 0.1560 ↔ `f68e590f` identity inferred (GEMSDOE2 upload order), not read from the account.
- Multiple accounts per project family (rules question for the owner).
- Public/private chunk geometry unpublished → algebra keeps two readings of E_i.
- The 3DEP VRT could not be opened from this sandbox (egress); the workflow's first step opens it and fails loudly.
...
7. **Next step 1's remaining human check closed from this sandbox.**
   `https://buffedlizard55-lab.github.io/5GEMSDOE/docs/submission_field.bin` serves the payload on
...
1. ~~Fire `make-submission.yml` once~~ **done twice.** Run 35805300948 (after the `.rc` fix) came
   ...
   `https://buffedlizard55-lab.github.io/5GEMSDOE/docs/submission_field.bin` (expect `200`,
...
9. **Both PRs merged to `main`.** [PR #23](https://github.com/buffedlizard55-lab/5GEMSDOE/pull/23)
   ...
   and a main-branch block-stratified measurement. [PR #24](https://github.com/buffedlizard55-lab/5GEMSDOE/pull/24)
...
1. **Executive summary subpage created and published:**
   [`docs/executive_summary.html`](https://buffedlizard55-lab.github.io/5GEMSDOE/docs/executive_summary.html)
...
[35134058898](https://github.com/buffedlizard55-lab/5GEMSDOE/actions/runs/35134058898), 16 s):
...
The reblend experiment (`reblend.yml`, run [35133590776](https://github.com/buffedlizard55-lab/5GEMSDOE/actions/runs/35133590776),
