...
|---|---|---|---|
| **`scripts/emit_by_marginal_rule.py` — STRATEGY.md H4, the re-budgeting script that was never written** | Implements R2 exactly: dT and dF come from `src.metrics.GtContext`'s running per-truth-pixel best-credit map, not an approximation. Two modes (`rule` = emit everything that raises the score; `budget` = the maximal clearing prefix), `--base` so the rule prices *additions* to a shipped field, `--price` for existing candidates | This is the one hypothesis whose delivery removes a whole class of floor/thinning sweeps that D3 disqualified. It needs **no input at all** — no G, no calibration, no upload — which is why it was the first thing built | `data/evidence/emission/marginal_rule_{salience,context}.json`; 13 tests in `tests/test_session3_budget_and_prior.py` |
| **The rule measures its own degeneracy instead of being argued with** | A pixel further than R = 3 px from every calibration-truth pixel has dT = 0, dF = 1 → marginal rate 0 → never admitted. With `labels.tif` as the instrument, the rule emits inside 300 m of the catalogue and **nowhere else** | H7's "no local instrument can price discovery" was a claim; this is the measurement. It is why the upload plan prices *additions* with `--base` and does not pretend the rule can choose a field | `docs/STRATEGY.md` §4c; the salience run's 63,645 chargeable px are all inside the corridor |
| **`scripts/train_context_detector.py` — a second detector family, on CPU, in six minutes** | HistGradientBoosting over the 19 official bands + derived |grad| and texture features, scored by **spatially blocked** folds (512 px super-regions, 3 px training collar) with the metric computed on global geometry; writes the field as **uint8 × 255**; a `--dtype {float32,uint8}` switch records the representation in the report | The CNN family needs a GPU (≈300 min/fold). This runs on 2 vCPU in **6 minutes**, carries the aux channels (the scarp channel) and produces a 3.9 MB committable artifact — the first detector family in this repo that can be produced, committed and re-scored entirely inside the sandbox | `data/evidence/context_detector.json`; `data/derived/context_detector_prob_u8.tif` |
| **`scripts/build_dilational_annulus.py` — thin the prior, then intersect with the detector** | Emits one halo pixel around each Faulds & Hinz setting (termination 25 %, intersection 22 %, bend 2 %, relay ramp 32 %) and keeps it only where the context detector fires ≥ q; 12-setting sweep written out | S5-B shipped the prior as the top-N of a *smoothed* field, i.e. a dense patch that pays FP everywhere it touches. The annulus form confines the FP cost to the halo, and intersecting with the detector is the evidence-layer combination the exploration literature itself uses — for **+8,053 chargeable px** over the 0.1563 base | `data/evidence/emission/dilational_annulus.json`; `docs/downloads/candidate_s5_dilational_annulus.tif` |
| **`scripts/verify_candidates_independently.py` + `scripts/vendor/gems_eval/` — a second, independent judge** | Every candidate in `docs/downloads/` is now gated by our 17-check validator *and* by a third-party implementation of the same rules (MIT, © 2026 Syntropy Digital, fetched 2026-09-25, unmodified, provenance in the directory). The two DTI implementations are cross-checked on the real rasters | "Verify no hallucinations" applied to our own metric code: an independent implementation of the spec is the strongest available check. On all six candidates both validators agree and the two DTIs agree to six decimals. The one failure (the blanket probe vs the vendored `plausible_coverage` heuristic) is recorded as a named exception, not silenced | `data/evidence/submission_independent_gate.json`; `tests/test_session3_budget_and_prior.py` |
| **The runner channel is unblocked without a human** | `fetch-gdr-inventory.yml` gained a push-path trigger (`.github/triggers/fetch-gdr`), matching what `build-topo-features` already had; the run's `--url` default now lives inside the `${{ }}` expression because a push event has empty inputs | `gh workflow run` is 403 and artifacts redirect to an unreachable Azure host, so session 2's two runner items were stranded. A push trigger fires on any push, costs one run, and is the only channel this token has | `.github/triggers/fetch-gdr`; `data/evidence/ci/pytest-36191990247.log`; run 36866533496 (failed on the empty `--url`, now fixed) |
| **`scripts/analyze_gdr355_inventory.py` — the geothermal truth set is now measured, and it flags a contradiction** | The runner's push-triggered fetch of GDR 355 landed this session (250,368 B, sha256 `843feb7c…`, sheets `StructureInventory` 426×23 and `Field Definitions`). This script reads those bytes and reports: **117 of the 426 systems fall inside the competition raster** (Beowawe, Dixie Valley, Steamboat, Bradys, Desert Peak and 112 more; mean max temperature 84.8 °C); the **published 32/25/22 % setting frequencies do not reproduce** on any denominator (measured 26.5 % undetermined, 18.5 % stepover, 14.6 % termination, 12.9 % intersection); and the **"39 % blind" claim is contradicted by the workbook's own field definition** (`Blind = yes` means the system *has* surface manifestations) | `labels.tif` is a fault catalogue and GDR 355 is a resource catalogue; the experts' new faults are by definition *not* in USGS/INGENIOUS, which puts them much closer to the latter. A 117-system geothermal prior inside the scored footprint is the first instrument of its kind in this project family, and it is testable with the `rank_instruments.py` question the SGMC proxy failed. The irregularity is recorded in the evidence JSON with both readings quoted, not resolved — the brief's "flag irregularities for review" | `data/evidence/gdr/inventory_analysis.json`; 3 tests in `tests/test_session3_budget_and_prior.py` |
| **Site staleness false positive fixed** | `tests/test_site_pages.py` now normalises the evidence JSON's `generated_at` prose stamp as well as the build stamp | Re-measuring any evidence file used to break the byte-comparison test, which made "regenerate before commit" unsafe | 6 passed |
...
4. **Executive Summary Delivered:** Created [`docs/executive_summary.html`](https://buffedlizard55-lab.github.io/5GEMSDOE/docs/executive_summary.html)
